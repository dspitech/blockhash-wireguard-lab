...
# ---------------------------------------------------------------------
def insert_alert(level, source, message, channels=None, ts=None):
    init_db()
    ts = ts if ts is not None else int(datetime.now(tz=timezone.utc).timestamp())
    with closing(get_conn()) as conn:
        conn.execute(
            "INSERT INTO alerts (ts, level, source, message, channels) VALUES (?, ?, ?, ?, ?)",
            (ts, level, source, message, json.dumps(channels or [])),
        )
        conn.commit()
    _fix_permissions()
